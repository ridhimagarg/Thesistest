...
def train(train_set, test_set, model_type, epochs, w_epochs, lr, ratio, factors, temperatures, 
            source_class, target_class, batch_size, w_lr, threshold, maxiter, shuffle, t_lr, dataset, distrib, verbose, extracted_lr):
...
#           dataset, distribution, verbose, extracted_lr):
    ...
    source_set, exclude_x_data_wm, exclude_y_data_wm = create_wm_dataset(distrib, x_train , y_train, source_class, target_class, dataset) ## need to handle for in distribution.
    ...
    if distrib == "out":
        ...
        for batch_idx, (trigger_input_label, target_input_label) in enumerate(zip(trigger_dl, target_dl)):
            ...
            grad = trainer_class.snnl_trigger(batch_data)
            trigger_grad.append(grad.detach().numpy())
        avg_grad = np.average(np.concatenate(trigger_grad), 0)
        print("avg grad", avg_grad.shape)
        down_sample = np.array([[np.sum(avg_grad[i: i + 3, j: j + 3]) for i in range(trigger_input_label[0].shape[1] - 2)] for j in range(trigger_input_label[0].shape[2] - 2)])
        print("downsample shape", down_sample.shape)
        w_pos = np.unravel_index(down_sample.argmin(), down_sample.shape)
