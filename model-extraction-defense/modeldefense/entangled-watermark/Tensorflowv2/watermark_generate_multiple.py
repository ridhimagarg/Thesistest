...
if __name__ == '__main__':
    ...
    parser.add_argument("-c", "--config", type=str, default="configs/watermark_set_multiple.yaml")
    ...
    for source in config["source"]:
        for target in config["target"]:
                for distribution in config["distribution"]:
                    ...
                    if "in" in distribution and source == target:

                        pass
                        
                    else:

                        if not os.path.exists(os.path.join(DATA_PATH, dataset_name + config["model_architecture"], distribution, str(target)+  str(source))):
                            os.makedirs(os.path.join(DATA_PATH, dataset_name + config["model_architecture"], distribution, str(target)+  str(source)))

                        generate_watermark(dataset_name, target, distribution, source, config["batch_size"], trigger_set_from_model_path, config["temperatures"],
