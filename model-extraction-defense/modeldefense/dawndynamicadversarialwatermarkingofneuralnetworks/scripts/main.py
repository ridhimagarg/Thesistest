...
Introduction:
    ...
    This file will be used to perform the training of the original model as well as to perform the attack.
    So, it can be used alone as to perform the training of the original model (victim model).
...
def main(config: configparser.ConfigParser) -> None:
    ...
    if env.problem_statement == "OBJECTDETECTION":
        ...
        if env.victim_retrain:
            ...
            plt.savefig(os.path.join(RESULTS_PATH, LOSS_FOLDER, config["DEFAULT"]["dataset_name"] + config["VICTIM"]["model_architecture"] + "Original_Loss.png" ))
            ...
            plt.savefig(os.path.join(RESULTS_PATH, LOSS_FOLDER, config["DEFAULT"]["dataset_name"] + config["VICTIM"]["model_architecture"] + "Original_Acc.png" ))
            ...
            # victim_model = env.training_ops.victim_model ## use this if want to use other models
            victim_model = tv.models.resnet34() ## for resnet34 we have used like this.

            n_features = victim_model.fc.in_features
            victim_model.fc = nn.Linear(n_features, 10)

            # victim_model = models.CIFAR10_BASE_2()

            ## taking the resnet model from thier repository itself.
            models.load_state(victim_model, "../data/models/victim_cifar_rn34.pt")

            # models.load_state(victim_model, "../models/original_18-09-2023/CIFAR10_BASE_2_victim_cifar_base_2.pt")

        ## checking the parameters of the victim model.

        # for name, param in victim_model.named_parameters():
        #     print(name, param)
        _ = agent.test_model(victim_model, test_data=True)
...
def handle_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config_file",
        type=str,
        default="../configurations/perfect/cifar-to-cifar-ws250-rn34.ini",
