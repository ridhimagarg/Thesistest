...
def validate_watermark(model_name, trigger_set, label, sess, batch_size, num_class, is_training, is_augment, x, y):
        ...
        error = sess.run(model_name.error, {x: trigger_data, y: labels, is_training: 0, is_augment: 0})[2]
        return 1 - error
...
def pca_and_plot(x, list_data, type_model, dataset, time, penultimate_layer, distrib):
    ...
    fig = plt.figure()
    plot = plt.scatter(transformed_data_train[:,0], transformed_data_train[:,1],  c="deepskyblue", marker=".", label="train data")
    plot = plt.scatter(transformed_data_target[:,0], transformed_data_target[:,1],  c="black", marker=",", label="target class")
    plot = plt.scatter(transformed_data_watermark[:,0], transformed_data_watermark[:,1],  c="darkorange", marker="<", label="watermark")
    plt.legend()
    ...
    plt.savefig(os.path.join(RESULTS_FOLDER, RESULTS_SUB_PCA_FOLDER, type_model + "_" + dataset + "_" +time+ "_" + str(penultimate_layer) + "_" + str(distrib)), dpi=fig.dpi)


def plot_activation(first_conv_legitimate, first_conv_watermark, second_conv_legitimate, second_conv_watermark, fc_legitimate, fc_watermark, type_model, dataset, distrib):
    ...
    plt.savefig(os.path.join(RESULTS_FOLDER, RESULTS_SUB_ACTIVATION_FOLDER, type_model + "_first_conv_legit_activation.png"))

    for i in range(10):
        fig.add_subplot(2,10, i+1)
        plt.imshow(first_conv_watermark[0,:,:,i], cmap="gray_r")
    plt.savefig(os.path.join(RESULTS_FOLDER, RESULTS_SUB_ACTIVATION_FOLDER, type_model + "_first_conv_watermark_activation.png"))

    fig = plt.figure(figsize=(10,10))
    for i in range(10):
        fig.add_subplot(2, 10, i+1)
        plt.imshow(second_conv_legitimate[0,:,:,i], cmap="gray_r")
    plt.savefig(os.path.join(RESULTS_FOLDER, RESULTS_SUB_ACTIVATION_FOLDER, type_model + "_second_conv_legit_activation.png"))

    for i in range(10):
        fig.add_subplot(2,10, i+1)
        plt.imshow(second_conv_watermark[0,:,:,i], cmap="gray_r")
    plt.savefig(os.path.join(RESULTS_FOLDER, RESULTS_SUB_ACTIVATION_FOLDER, type_model + "_second_conv_watermark_activation.png"))
    ...
    plt.savefig(os.path.join(RESULTS_FOLDER, RESULTS_SUB_ACTIVATION_FOLDER, type_model + "_" + dataset + "_" + distrib + "_fc_activation.png"))
