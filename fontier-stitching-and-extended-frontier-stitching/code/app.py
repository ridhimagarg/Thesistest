...
if uploaded_files is not None:
    ...
    for uploaded_file in uploaded_files:
        ...
        if "original" in uploaded_file.name:
            st.info(f"Original Model Prediction on Normal Sample: **{labels_mapping[np.argmax(prediction)]}**")
        else:
            st.info(f"Original Model Prediction on Watermarked Sample: **{labels_mapping[np.argmax(prediction)]}**")

        prediction = finetuned_model.predict(np.expand_dims(numpy_arr['arr_0'], axis=0))
        # st.info(f"Watermarked Model Prediction on Normal Data: **{labels_mapping[np.argmax(prediction)]}**")
        if "original" in uploaded_file.name:
            st.info(f"Watermarked (Finetuned) Model Prediction on Normal Sample: **{labels_mapping[np.argmax(prediction)]}**")
        else:
            st.info(f"Watermarked (Finetuned) Model Prediction on Watermarked Sample: **{labels_mapping[np.argmax(prediction)]}**")

        st.write("Claiming ownership of the stolen model")
        prediction = attack_finetuned_model.predict(np.expand_dims(numpy_arr['arr_0'], axis=0))
        # st.info(f"Stolen Model Prediction: **{labels_mapping[np.argmax(prediction)]}**")
        if "original" in uploaded_file.name:
            st.info(f"Stolen Model Prediction on Normal Sample: **{labels_mapping[np.argmax(prediction)]}**")
        else:
            st.info(f"Stolen Model Prediction on Watermarked Sample: **{labels_mapping[np.argmax(prediction)]}**")
